...
/**
 * @file DVDlancamento.hpp
 * @brief Arquivo de cabeçalho no qual contém os atributos e a especifição dos métodos utilizados no arquivo "DVDlancamemto.cpp".
 * @details Esse arquivo define a classe na qual contém os métodos que serão utilizados no desenvolvimento do 
 * código do arquivo "DVDlancamento.cpp", especifica o tipo desses métodos (o que eles irão retornar) e os parâmetros que 
 * serão recebidos por eles.
 * @class dvdLancamento
 * @details Na classe "dvdLancamento", que herda publicamente a classe "Filme", existem como atributos públicos um construtor, o qual
 ...
 * ("dvdLancamento(int id, string titulo, int unidades)"). Existe também um método para calcular quanto será pago pelo aluguel do filme
